...
start:
        ...
        mov si, novapad_title
        call os_print_string

        mov si, mode_question
        call os_print_string
...
create_editor:
        ; اینجا بعداً بخش ادیتور اصلی می‌آید
        jmp render_text
...
file_chosen:
	...
	; NovaPad accepts all text files
...
save_file:
        ...
        mov ax, filename
        call os_remove_file            ; Delete existing file first — os_write_file
                                        ; refuses to touch a file that already exists
...
; Data section
	...
	txt_title_msg	db 'NovaPad Text Editor', 0
	...
	novapad_title db 'NovaPad',0
	mode_question db 13,10
              db '1 - Load existing file',13,10
              db '2 - Create new file',13,10
              db 'Choice: ',0

	filename_question db 13,10
                  db 'New filename: ',0
	create_fail_msg db 'Cannot create file!',0
; ------------------------------------------------------------------
